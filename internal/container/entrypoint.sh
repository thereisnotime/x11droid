...
put_global() {
  ...
  for i in $(seq 1 12); do
    ...
    # The `--` is essential: `waydroid shell` is argparse-based and otherwise
    # steals su's `-c` flag ("unrecognized arguments: -c ..."); `--` passes the
    # whole su command through untouched. Fall back to a plain root put for
    # unguarded keys (e.g. device_name) / images without su.
